...
\newpage
	...
	\end{center}
	\begin{itemize}
	\item We will be using the Waterfall Methodology (also known as linear-sequential life cycle model) which is a linear approach to software development. The reason for choosing to go with this process is because the project itself is quite large, and thus in order to do every part of the project correctly and efficiently we will need some sort of consistency which is what the Waterfall methodology guarantees, as we as the developers will agree on what will be delivered early in the development lifecycle. This makes planning and designing more straightforward. And also progress is more easily measured, as the full scope of the work is known in advance.
	\end{itemize}
...
\begin{center}
	...
	\end{center}
		\begin{itemize}
\item We will try to use an image identification library such as OpenCV, an open source library aimed at real-time computer vision, to help with the detection of people in photos. Using multiple photos we will try to optimize the parameters at which it operates in order to maximize its accuracy.
\end{itemize}
\begin{center}
